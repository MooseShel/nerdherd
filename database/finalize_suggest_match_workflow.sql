-- ==============================================================================
-- FINALIZE SUGGEST_MATCH WORKFLOW
-- ==============================================================================

-- LOGIC:
-- 1. Check if users are perm-connected in 'public.connections'.
-- 2. If Connected: Auto-Accept Match (Straight to Chat).
-- 3. If NOT Connected: Force Reset Match to 'Pending' (Handshake Required).
--    (This applies even if they matched before. New SOS = New Handshake).
...
BEGIN
    ...
    IF v_match_id IS NULL THEN
        ...
        VALUES (
            ...
            target_user_id, 
            match_type,
            v_are_connected, -- True if connected, False if not
            v_are_connected  -- True if connected, False if not
        )
    ...
    ELSE
        ...
        -- Update the existing record to reflect current connection status.
        -- If connected -> Accepted = True (Fast Track)
        -- If NOT connected -> Accepted = False (Force Handshake)
    ...
    -- 3. FORCED UPSERT: Sync Collab Request state
